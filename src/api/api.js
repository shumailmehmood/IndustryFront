...
export const postPayment = async (data) => {
    return await resolve(axios.post(`${apiBaseUrl}/api/v1/transaction`, data)
        .then(res => res.data));
}
//==========================GET========================//
...
export const get_Account = async (params) => {
    return await resolve(axios.get(`${apiBaseUrl}/api/v1/get_payment?from=${params.date}&name=${params.name}&page=${params.page}&limit=${params.limit}`)
        .then(res => res.data));
}
